...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/js/bootstrap.min.js" integrity="sha384-j0CNLUeiqtyaRmlzUHCPZ+Gy5fQu0dQ6eZ/xAww941Ai1SxSY+0EQqNXNE6DZiVc" crossorigin="anonymous"></script>
    ...
    function byid(id_riwayat, type){
      ...
      $.ajax({
         type: "POST",
                 url: "<?= base_url('serverside/byid/') ?>" +id_riwayat ,
                 dataType: "JSON",
                  ...
                  if (type=='detail') {
                    ...
                    var no = 1;
                        for (i = 0; i < data.length; i++) {
                            $("#detail").append('<tr><td>' + no + '</td><td>' + data[i]['kebun'] + '</td><td>' + data[i]['nama_plan'] + '</td><td>' + data[i]['luas_plan'] +  '</td></tr>');
                            no++;
                            modal.modal('show');
                               }   
                              } else{
    ...
    function deleteData(id_riwayat){
      ...
      $.ajax({
                 type: "POST",
                 url: "<?= base_url('serverside/delete_riwayat/') ?>" +id_riwayat ,
                 dataType: "JSON",
